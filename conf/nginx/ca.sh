...
if [ -e "/usr/local/nginx/conf/ssl/moss.pem" ];then
    ...
    umask 77
    openssl req -utf8 -newkey rsa:2048 -keyout /usr/local/nginx/conf/ssl/moss.key -nodes -x509 -days 3650 -out /usr/local/nginx/conf/ssl/moss.crt -set_serial 0 -subj "/C=CN/ST=Moss/L=Moss/O=Moss Inc/CN=$VHOST_DN"
    ...
    openssl dhparam -out /usr/local/nginx/conf/ssl/dh2048.pem 2048
fi
